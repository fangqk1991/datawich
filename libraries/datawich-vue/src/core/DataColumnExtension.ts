...
import { FieldActionParams, FieldLinkModel, GeneralDataHelper, ModelFieldModel } from '@fangcha/datawich-service'
    ...
    <span>
      <template v-if="!Boolean(superField)">
        <a v-for="action in linkActions" :key="action.actionId" :href="makeLink(action)" target="_blank">
          <el-tag v-if="makeLink(action)" class="adaptive-tag" type="danger"
            ><i class="el-icon-connection" /> {{ action.title }}</el-tag
          >
        </a>
      </template>
...
export class DataColumnExtension extends ViewController {
  ...
  makeLink(action: FieldActionParams) {
